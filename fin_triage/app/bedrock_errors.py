...
(`app.main`) quanto o HTTP 502 dos endpoints (`app.routers.triage`) usam isto
...
Cópia generalizada da versão do `fin_guardrail`: o parâmetro `resource` é o
recurso Bedrock em jogo (aqui, o model id) em vez de um guardrail, e as
mensagens falam em `bedrock:InvokeModel`.
    ...
    *,
    ...
    if aws_code in _NOT_FOUND_CODES or (
        ...
        return "BEDROCK_RECURSO_NAO_ENCONTRADO", (
            ...
            "Verifique BEDROCK_MODEL_TRIAGE e AWS_REGION no .env, e se o modelo está "
            "habilitado na conta."
        )
